'use strict';

define(['jquery', 'react', 'jsx!widgets/Unit-Input', 'jsx!widgets/Dialog-Menu', 'helpers/api/config', 'helpers/i18n'], function ($, React, UnitInput, DialogMenu, config, i18n) {
    'use strict';

    var lang = i18n.lang;

    return React.createClass({

        getDefaultProps: function getDefaultProps() {
            return {
                defaults: {},
                imageFormat: 'svg' // svg, bitmap
            };
        },

        getInitialState: function getInitialState() {
            return {
                defaults: this.props.defaults
            };
        },

        isShading: function isShading() {
            return false;
        },

        // UI Events
        _saveLastestSet: function _saveLastestSet() {
            var self = this,
                refs = self.refs,
                opts = {
                liftHeight: refs.liftHeight.value(),
                drawHeight: refs.drawHeight.value(),
                speed: refs.speed.value()
            },
                state = {
                defaults: opts
            };

            config().write('draw-defaults', opts);

            self.setState(state);
        },

        openSubPopup: function openSubPopup(e) {
            this.refs.dialogMenu.toggleSubPopup(e);
        },

        _updateDefaults: function _updateDefaults(e, value) {
            this._saveLastestSet();
            this.openSubPopup(e);
        },

        // Lifecycle
        _renderLiftHeight: function _renderLiftHeight() {
            var min = Math.max(5, this.state.defaults.drawHeight);

            return {
                label: React.createElement(
                    'div',
                    { title: lang.draw.pen_up_title },
                    React.createElement(
                        'span',
                        { className: 'caption' },
                        lang.draw.pen_up
                    ),
                    React.createElement(
                        'span',
                        null,
                        this.state.defaults.liftHeight
                    ),
                    React.createElement(
                        'span',
                        null,
                        lang.draw.units.mm
                    )
                ),
                content: React.createElement(
                    'div',
                    { className: 'object-height-input' },
                    React.createElement(UnitInput, {
                        ref: 'liftHeight',
                        defaultUnit: 'mm',
                        defaultValue: this.state.defaults.liftHeight,
                        getValue: this._updateDefaults,
                        min: min,
                        max: 150
                    })
                )
            };
        },

        _renderDrawHeight: function _renderDrawHeight() {
            var max = Math.min(150, this.state.defaults.liftHeight);

            return {
                label: React.createElement(
                    'div',
                    { title: lang.draw.pen_down_title },
                    React.createElement(
                        'span',
                        { className: 'caption' },
                        lang.draw.pen_down
                    ),
                    React.createElement(
                        'span',
                        null,
                        this.state.defaults.drawHeight
                    ),
                    React.createElement(
                        'span',
                        null,
                        lang.draw.units.mm
                    )
                ),
                content: React.createElement(
                    'div',
                    { className: 'object-height-input' },
                    React.createElement(UnitInput, {
                        ref: 'drawHeight',
                        defaultUnit: 'mm',
                        defaultValue: this.state.defaults.drawHeight,
                        getValue: this._updateDefaults,
                        min: 5,
                        max: max
                    })
                )
            };
        },

        _renderSpeed: function _renderSpeed() {
            return {
                label: React.createElement(
                    'div',
                    { title: lang.draw.speed_title },
                    React.createElement(
                        'span',
                        { className: 'caption' },
                        lang.draw.speed
                    ),
                    React.createElement(
                        'span',
                        null,
                        this.state.defaults.speed
                    ),
                    React.createElement(
                        'span',
                        null,
                        lang.draw.units.mms
                    )
                ),
                content: React.createElement(
                    'div',
                    { className: 'object-height-input' },
                    React.createElement(UnitInput, {
                        ref: 'speed',
                        defaultUnit: 'mm/s',
                        defaultUnitType: 'speed',
                        defaultValue: this.state.defaults.speed,
                        getValue: this._updateDefaults,
                        min: 0.8,
                        max: 150
                    })
                )
            };
        },

        render: function render() {
            var liftHeight = this._renderLiftHeight(),
                drawHeight = this._renderDrawHeight(),
                speed = this._renderSpeed(),
                items = [liftHeight, drawHeight, speed];

            return React.createElement(
                'div',
                { className: 'setup-panel operating-panel' },
                React.createElement(DialogMenu, { ref: 'dialogMenu', items: items })
            );
        }

    });
});