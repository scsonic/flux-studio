...
define(['jquery', 'helpers/three/to-screen-position', 'helpers/three/matrix', 'threejs', 'threeOrbitControls', 'threeTransformControls', 'threeCircularGridHelper', 'threeSTLLoader'], function ($, _toScreenPosition, matrix) {
    'use strict';
    ...
    function addCircularGridHelper() {
        ...
        var circularGridHelper = new CircularGridHelper(settings.diameter, settings.step, settings.upVector, settings.color, settings.opacity, settings.text, settings.textColor, settings.textPosition, 0);
    ...
    function refreshPosition(mesh, objectChange, delay) {
        return function () {
            var matrixValue = matrix(mesh),
                timer,
                objectScreenPosition = _toScreenPosition(mesh, camera, container);
    ...
    function attachControl(mesh, objectChange, delay) {
        ...
        var setMode = function setMode(mode) {
            ...
            createTransformControls = function createTransformControls(mesh) {
    ...
    function setupGeometry(model_data, geometry) {
        var SET_ELEM_NUMBER = 6,
            ...
            vertices = new Float32Array(number_sets * 3),
        ...
        for (var i = 0; i < model_data.length; i += SET_ELEM_NUMBER) {
            ...
            color.setRGB(model_data[i + RED], model_data[i + GREEN], model_data[i + BLUE]);
    ...
    return {
        ...
        toScreenPosition: function toScreenPosition(mesh) {
            return _toScreenPosition(mesh, camera, container);
        },
