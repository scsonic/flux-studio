...
define(['jquery', 'helpers/i18n', 'helpers/local-storage', 'helpers/shortcuts', 'helpers/api/config', 'helpers/logger', 'helpers/device-master', 'app/actions/alert-actions', 'app/stores/alert-store'], function ($, i18n, localStorage, shortcuts, config, Logger, DeviceMaster, AlertActions, AlertStore) {
    'use strict';
    ...
    var genericLogger = new Logger('generic'),
        defaultKeyBehavior = function defaultKeyBehavior() {
        shortcuts.on(['BACK'], function (e) {
            ...
            if ('INPUT' === me.tagName && -1 < acceptedInput.indexOf(inputType) || 'TEXTAREA' === me.tagName) {
                try {
                ...
                deleteCount = true === samePosition ? // same position
                1 : e.target.selectionEnd - selectionStart;
                deleteStart = true === samePosition ? // same position
        ...
        shortcuts.on([FN_KEY, 'c'], function () {
            window.document.execCommand('copy');
        });
        shortcuts.on([FN_KEY, 'a'], function () {
            window.document.execCommand('selectAll');
        });
        shortcuts.on([FN_KEY, 'x'], function () {
            window.document.execCommand('cut');
        });
        shortcuts.on([FN_KEY, 'v'], function () {
            window.document.execCommand('paste');
        });
        shortcuts.on([FN_KEY, '0'], function () {
            console.log("Reset View!");window.svgEditor.resetView();
        });
        shortcuts.on([FN_KEY, 'plus'], function () {
            console.log("Zoom In");window.svgEditor.zoomIn();
        });
        shortcuts.on([FN_KEY, 'minus'], function () {
            console.log("Zoom Out");window.svgEditor.zoomOut();
        });
    ...
    window.FLUX.processPythonException = function (exception_str) {
        if (exception_str.indexOf("ImportError: DLL load failed") !== -1) {
            AlertActions.showPopupError('error-vcredist', lang.support.no_vcredist);
        }
    };

    //Process OS X Version Check
    if (window.navigator.userAgent.indexOf("Intel Mac OS X 10_9") !== -1) {
        AlertActions.showPopupError('error-osx_10_9', lang.support.osx_10_9);
    };
