...
define(['jquery', 'helpers/api/bitmap-laser-parser', 'helpers/api/svg-laser-parser', 'helpers/api/fcode-reader', 'helpers/convertToTypedArray', 'helpers/element-angle', 'helpers/sprintf', 'helpers/api/control', 'helpers/shortcuts', 'helpers/image-data', 'helpers/i18n', 'helpers/round', 'app/actions/alert-actions', 'app/actions/global-actions', 'app/constants/global-constants', 'helpers/device-master', 'app/constants/device-constants', 'app/actions/progress-actions', 'app/constants/progress-constants', 'helpers/device-error-handler', 'helpers/check-device-status', 'freetrans', 'helpers/jquery.box', 'plugins/file-saver/file-saver.min'], function ($, bitmapLaserParser, svgLaserParser, fcodeReader, convertToTypedArray, elementAngle, sprintf, control, shortcuts, imageData, i18n, round, AlertActions, GlobalActions, GlobalConstants, DeviceMaster, DeviceConstants, ProgressActions, ProgressConstants, DeviceErrorHandler, CheckDeviceStatus) {
    'use strict';
    ...
    return function (args) {
        ...
        var self = this,
            // react component
        IMAGE_REAL_RATIO = 4.7244094488,
            // 1mm = 4.72px
        DIAMETER = 170,
            // 170mm
        ACCEPTABLE_MIN_SIZE = 1,
            // width * height > 1
        bitmapWebSocket,
            svgWebSocket,
            ...
            _onUploadResponse = function _onUploadResponse(response) {
            var url = window.URL,
            ...
            response.files.forEach(function (file, i) {
                ...
                if ('good' === file.status && ACCEPTABLE_MIN_SIZE > file.imgSize.width * file.imgSize.height) {
                    file.status = 'bad';
                ...
                if ('bad' === file.status) {
                    ...
                    file.messages.forEach(function (errorCode, i) {
            ...
            _onUploaded = function _onUploaded(response) {
            var goodFiles = response.files.filter(function (file) {
            ...
            refreshImage = function refreshImage($img, threshold) {
            ...
            sendToMachine = function sendToMachine(blob) {
            var blobUrl = window.URL,
                source = (self.props.page || '').toUpperCase(),
                fcodeReaderMethods = fcodeReader(),
                goToMonitor = function goToMonitor(thumbnailBlob) {
                ProgressActions.close();
                ...
                parseFCode = function parseFCode() {
                fcodeReaderMethods.getThumbnail().then(function (data) {
                    goToMonitor(data);
                });
            },
                uploadFCode = function uploadFCode() {
                fcodeReaderMethods.upload(blob).then(function () {
                    parseFCode();
                });
            };
            ...
            ExportGCodeProgressing = function ExportGCodeProgressing(data) {
            ProgressActions.open(ProgressConstants.STEPPING);
            ProgressActions.updating(data.message, data.percentage * 100);
        },
            sendToBitmapAPI = function sendToBitmapAPI(args, settings, callback, fileMode) {
            callback = callback || function () {};

            var laserParser = bitmapWebSocket,
                onSetParamsFinished = function onSetParamsFinished() {
                laserParser.compute(args).done(onUploadFinish);
            },
                onUploadFinish = function onUploadFinish() {
            ...
            laserParser.clear().done(function (data) {
                laserParser.params.setEach(settings, {
                    onFinished: onSetParamsFinished
                });
            });
        },
            sendToSVGAPI = function sendToSVGAPI(args, settings, callback, fileMode) {
            callback = callback || function () {};
            ...
            var laserParser = svgWebSocket,
                ...
                onComputeFinished = function onComputeFinished() {
                var names = [],
            ...
            convertToRealCoordinate = function convertToRealCoordinate(px, axis) {
            axis = axis || '';
            ...
            convertToHtmlCoordinate = function convertToHtmlCoordinate(n, axis) {
            var ratio = PLATFORM_DIAMETER_PIXEL / DIAMETER,
                // 1(px) : N(mm)
            r = DIAMETER / 2,
            ...
            outOfRange = function outOfRange(point) {
            ...
            resetPosition = function resetPosition($target) {
            ...
            $img_container.each(function (i, el) {
                ...
                $controlPoints.each(function (k, el) {
                    ...
                    rollback = false === outOfRange(elPosition.center) ? false : rollback;
                });
            ...
            refreshObjectParams = function refreshObjectParams(e, $el, returnState) {
            ...
            if (null !== $el) {
                ...
                var x = convertToRealCoordinate(el_position.center.x, 'x'),
            ...
            $target_image = null,
            // changing when image clicked
        resetPosTimer = null,

        //============== for test async function ===========================
        showOutline = async function showOutline(object_height, outLine_data) {
            await DeviceMaster.select(self.state.selectedPrinter);
            ProgressActions.open(ProgressConstants.WAITING, lang.device.showOutline);
        ...
        getPoint = function getPoint($el) {
            ...
            getToolpath = function getToolpath(settings, callback, progressType, fileMode) {
            fileMode = fileMode || '-f';
            ...
            //================ END ==================================*/
            args = [],
                doLaser = function doLaser(settings) {
        ...
        function setupImage(file, size, originalUrl) {
            var img = new Image(),
                ...
                instantRefresh = function instantRefresh(e, data) {
                refreshObjectParams(e, $img);
            },
                $ftControls;

            $img.addClass(file.extension).attr('src', file.url).data('name', file.uploadName).data('base', originalUrl).data('file', file).data('size', size).data('sizeLock', true).width(size.width).height(size.height);
                ...
                (function (file, size, originalUrl, $img) {
                    ...
                    $ftControls.on('mousedown', function (e) {
                        ...
                        if (false === $img.hasClass('image-active')) {
                            ...
                            $target_image = $img;
                            var objectParamState = refreshObjectParams({ freetransEventType: 'move' }, $img, true);
        ...
        function inactiveAllImage($exclude, dontRefresh) {
            var $imageActive = $('img.image-active');
            if ($exclude) {
        ...
        var laserHelper = {
            runCommand: function runCommand(settings, command) {
                if (command === 'start') {
                    getToolpath(settings, sendToMachine, ProgressConstants.STEPPING);
                    ...
                    $ft_controls.each(function (index, image) {
                        var $image = $(image),
                            ...
                            position = {
                            first: [convertToRealCoordinate(tl.x, 'x'), convertToRealCoordinate(tl.y, 'y')],
                            second: [convertToRealCoordinate(tr.x, 'x'), convertToRealCoordinate(tr.y, 'y')],
                            third: [convertToRealCoordinate(br.x, 'x'), convertToRealCoordinate(br.y, 'y')],
                            fourth: [convertToRealCoordinate(bl.x, 'x'), convertToRealCoordinate(bl.y, 'y')]
                        };
                        positions.push(position);
                    });
                    showOutline(objectHeight, positions);
                } else if (command === 'calibrate') {
                    DeviceMaster.select(self.state.selectedPrinter).then(function (printer) {
                        setTimeout(function () {
                            ProgressActions.open(ProgressConstants.NONSTOP, lang.cut.running_horizontal_adjustment);
                        }, 1);
                        CheckDeviceStatus(self.state.selectedPrinter).then(function () {
                            DeviceMaster.calibrate({ forceExtruder: false, doubleZProbe: false, withoutZProbe: true }).done(function (debug_message) {
                                setTimeout(function () {
                    ...
                    DeviceMaster.select(self.state.selectedPrinter).then(function (printer) {
                        setTimeout(function () {
                            ProgressActions.open(ProgressConstants.NONSTOP, lang.cut.running_height_adjustment);
                        }, 1);
                        CheckDeviceStatus(self.state.selectedPrinter).then(function () {
                            DeviceMaster.zprobe({ forceExtruder: false }).done(function (debug_message) {
                                setTimeout(function () {
            ...
            exportTaskCode: function exportTaskCode(settings, fileMode) {
                getToolpath(settings, function (blob, fileMode) {
                ...
                }, ProgressConstants.STEPPING, fileMode || '-f');
            },
            ...
            resetFileFormat: function resetFileFormat() {
                self.setState({
                    fileFormat: undefined
                });
            },
            uploadDefaultLaserImage: function uploadDefaultLaserImage() {
            ...
            onReadFileStarted: function onReadFileStarted(e) {
            ...
            onFileReadEnd: function onFileReadEnd(e, files) {
            ...
            thresholdChanged: function thresholdChanged(threshold) {
            ...
            imageTransform: function imageTransform(e, params) {
            ...
            setPlatform: function setPlatform(el) {
                $laser_platform = $(el);
