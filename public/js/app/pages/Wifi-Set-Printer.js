...
define(['react', 'reactDOM', 'reactClassset', 'app/actions/initialize-machine', 'helpers/api/usb-config', 'jsx!widgets/Modal', 'app/actions/alert-actions', 'app/stores/alert-store', 'helpers/api/config', 'helpers/api/upnp-config', 'helpers/device-master', 'app/actions/progress-actions', 'app/constants/progress-constants', 'helpers/device-error-handler'], function (React, ReactDOM, ReactCx, initializeMachine, usbConfig, Modal, AlertActions, AlertStore, Config, upnpConfig, DeviceMaster, ProgressActions, ProgressConstants, DeviceErrorHandler) {
    'use strict';
    ...
    return function (args) {
        ...
        return React.createClass({
            ...
            _handleSetPrinter: function _handleSetPrinter(e) {
                e.preventDefault();
                ...
                var self = this,
                    ...
                    startSettingWithUsb = function startSettingWithUsb() {
                    usb = usbConfig();
                    ...
                    startSettingWithWifi = function startSettingWithWifi() {
                    ...
                    setMachine,
                    isValidName,
                    settingPrinter = function settingPrinter() {
                    AlertStore.removeYesListener(settingPrinter);
                    AlertStore.removeNoListener(cancelSetting);
            ...
            _renderAlert: function _renderAlert(lang) {
                var self = this,
                    buttons = [{
                    ...
                    content = React.createElement(Alert, { caption: this.state.alertContent.caption, message: this.state.alertContent.message, buttons: buttons });

                return true === this.state.openAlert ? React.createElement(Modal, { content: content }) : '';
            },
            ...
            render: function render() {
                ...
                oldPassword = true === this.state.settingPrinter.password && '' === (this.state.settingPrinter.plaintext_password || '') && 'WIFI' === this.state.settingPrinter.from ? React.createElement(
                    'label',
                    { className: 'control', htmlFor: 'printer-old-password' },
                    React.createElement(
                        'h4',
                        { className: 'input-head padleft' },
                        lang.initialize.set_machine_generic.old_password
                    ),
                ...
                content = React.createElement(
                    ...
                    React.createElement(
                        'form',
                        { className: 'form h-form', onSubmit: this._handleSetPrinter },
                        React.createElement(
                            'h1',
                            { className: 'headline' },
                            lang.initialize.name_your_flux
                        ),
                        React.createElement(
                            'div',
                            { className: 'controls' },
                            React.createElement(
                                'label',
                                { className: 'control', htmlFor: 'printer-name' },
                                React.createElement(
                                    'h4',
                                ...
                                React.createElement(
                                    ...
                                    invalidPrinterNameMessage
                                )
                            ),
                            oldPassword,
                            React.createElement(
                                'label',
                                { className: 'control', htmlFor: 'printer-password' },
                                React.createElement(
                                    'h4',
                                    { className: 'input-head' },
                        ...
                        React.createElement(
                            ...
                            React.createElement(
                                'button',
                                { type: 'submit', className: 'btn btn-action btn-large', 'data-ga-event': 'next' },
