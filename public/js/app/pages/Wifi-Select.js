...
define(['jquery', 'react', 'reactClassset', 'reactDOM', 'app/actions/initialize-machine', 'jsx!widgets/Modal', 'jsx!widgets/List', 'jsx!widgets/Button-Group', 'jsx!widgets/Alert', 'helpers/api/usb-config', 'helpers/api/upnp-config', 'app/actions/progress-actions', 'app/constants/progress-constants', 'app/actions/alert-actions', 'app/stores/alert-store'], function ($, React, ReactCx, ReactDOM, initializeMachine, Modal, ListView, ButtonGroup, Alert, usbConfig, upnpConfig, ProgressActions, ProgressConstants, AlertActions, AlertStore, DeviceErrorHandler) {
    'use strict';
    ...
    return function (args) {
        ...
        return React.createClass({
            ...
            _afterStopWifiScanning: function _afterStopWifiScanning(args) {
                var self = this;
            ...
            _goToSetPassword: function _goToSetPassword() {
            ...
            _setApMode: function _setApMode() {
                var self = this,
                    settingPrinter = self.state.settingPrinter,
            ...
            _setApModeViaUsb: function _setApModeViaUsb(name, pass) {
                var self = this,
            ...
            _setApModeViaWifi: function _setApModeViaWifi(name, pass) {
                var self = this,
                    lang = self.state.lang,
                    settingPrinter = self.state.settingPrinter;
            ...
            _settingWifiViaWifi: function _settingWifiViaWifi() {
            ...
            _setWifiWithoutPassword: function _setWifiWithoutPassword() {
            ...
            // UI events
            _confirmWifi: function _confirmWifi(e) {
                e.preventDefault();
            ...
            _stopScan: function _stopScan(action) {
                this.action = action;
                this.deferred.notify('STOP_SCAN');
            },

            _startScan: function _startScan() {
                this.action = '';
                this.deferred.notify('SCAN_WIFI');
            },

            _selectWifi: function _selectWifi(e) {
            ...
            _checkApModeSetting: function _checkApModeSetting(e) {
            ...
            _setAsStationMode: function _setAsStationMode(e) {
                e.preventDefault();
            ...
            _joinNetwork: function _joinNetwork(e) {
                e.preventDefault();
                ...
                var wifi = { ssid: ssid, security: security };
                initializeMachine.settingWifi.set(wifi);
                this._stopScan();
                usbSocket.joinWifiNetwork(wifi, wepkey, true).then(function (result) {
                    if (result.status === 'ok') {
                        location.hash = '#initialize/wifi/notice-from-device';
                    }
                }).fail(function (error) {
            ...
            _renderPasswordForm: function _renderPasswordForm(lang) {
                var self = this,
                    settingWifi = initializeMachine.settingWifi.get(),
                    buttons = [{
                    ...
                    'form',
                    { className: 'form form-wifi-password', onSubmit: self._handleSetPassword },
                    React.createElement(
                        'div',
                        { className: 'notice' },
                        React.createElement(
                        ...
                        React.createElement(
                            'p',
                            null,
                            lang.initialize.requires_wifi_password
                        )
                    ),
                ...
                return true === this.state.openPassword ? React.createElement(Modal, { content: content }) : '';
            },

            _renderApModeForm: function _renderApModeForm(lang) {
                ...
                var self = this,
                    ...
                    content = React.createElement(
                    'form',
                    { className: 'form form-ap-mode', onSubmit: self._setAsStationMode },
                    React.createElement(
                        'h2',
                        null,
                        lang.initialize.set_machine_generic.create_network
                    ),
                    React.createElement(
                        'label',
                        { className: 'h-control' },
                        ...
                        }, _defineProperty(_React$createElement, 'placeholder', lang.initialize.set_machine_generic.ap_mode_name_placeholder), _defineProperty(_React$createElement, 'onChange', self._checkApModeSetting), _React$createElement))
                    ),
                    React.createElement(
                        'label',
                        { className: 'h-control' },
                        ...
                        }, _defineProperty(_React$createElement2, 'placeholder', lang.initialize.set_machine_generic.ap_mode_pass_placeholder), _defineProperty(_React$createElement2, 'onChange', self._checkApModeSetting), _React$createElement2))
                    ),
                    React.createElement(
                        'div',
                        ...
                        React.createElement(
                            'button',
                            { className: 'btn btn-action btn-large', type: 'submit' },
                            lang.initialize.confirm
                        ),
                        React.createElement(
                            'button',
                            { className: 'btn btn-action btn-large btn-link', onClick: closeForm },
                            lang.initialize.cancel
                        )
                    )
                );

                return true === this.state.openApModeForm ? React.createElement(Modal, { content: content }) : '';
            },

            _renderJoinNetworkForm: function _renderJoinNetworkForm(lang) {
                var self = this,
                    closeForm = function closeForm(e) {
                    self.setState({
                        openJoinNetworkForm: false
                    ...
                    React.createElement(
                        'h2',
                        null,
                        lang.initialize.set_machine_generic.join_network
                    ),
                    React.createElement(
                        'label',
                        { className: 'h-control' },
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            'select',
                            { ref: 'network_security', className: 'security' },
                            React.createElement(
                                'option',
                                { value: 'NONE' },
                                'NONE'
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            'button',
                            { className: 'btn btn-action btn-large', type: 'submit', onClick: this._joinNetwork },
                            lang.initialize.confirm
                        ),
                        React.createElement(
                            'button',
                ...
                return this.state.openJoinNetworkForm ? React.createElement(Modal, { content: content }) : '';
            },

            _renderWifiItem: function _renderWifiItem(wifi) {
                var settingWifi = initializeMachine.settingWifi.get(),
                ...
                return React.createElement(
                    'label',
                    { 'data-meta': meta },
                    React.createElement('input', { type: 'radio', name: 'wifi-spot', value: wifi.ssid, defaultChecked: settingWifi.ssid === wifi.ssid }),
                    React.createElement(
                        'div',
                        { className: 'row-fluid' },
                        React.createElement(
                        ...
                        React.createElement('span', { className: lockClassName }),
                        React.createElement('span', { className: 'wifi-signal-strength fa fa-wifi' })
                    )
                );
            },

            _renderWifiOptions: function _renderWifiOptions(lang) {
                return this.state.wifiOptions.length > 0 ? React.createElement(ListView, {
                    ref: 'wifiList',
                ...
                var self = this,
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            'h1',
                            { className: 'headline' },
                            lang.initialize.wifi_setup
                        ),
                        React.createElement(
                            'p',
                            { className: 'notice' },
                            lang.initialize.select_preferred_wifi
                        ),
                        items,
                        React.createElement(ButtonGroup, { className: 'btn-v-group', buttons: buttons })
                    ),
