 ...
 * output error log
 */
define(['jquery', 'helpers/i18n', 'html2canvas', 'helpers/logger', 'plugins/file-saver/file-saver.min'], function ($, i18n, html2canvas, Logger) {
    'use strict';
    ...
    return function () {
        ...
        html2canvas(window.document.body).then(function (canvas) {
            ...
            if (FLUX.logfile) {
                var fs = require("fs");
                try {
                    var buf = fs.readFileSync(FLUX.logfile, { encoding: "utf8" });
                    output.push('\n\n======::backend::======\n');
                    output.push(buf);
                } catch (err) {
                    output.push('\n\n======::backend::======\n');
