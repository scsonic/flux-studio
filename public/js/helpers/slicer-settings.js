...
define(['jquery', 'app/default-print-settings'], function ($, DefaultPrintConfigs) {

    var CONFIG_FLUX_TO_CURA = {
        ...
        fill_density: { key: 'infill_line_distance', fn: function fn(v, settings) {
                v = v || '10%';return v === '0%' ? 0 : 0.4 * 100 / parseFloat(v.toString().replace('%', ''));
            } },
        fill_pattern: { key: 'infill_pattern', fn: function fn(v) {
                v = v.toLowerCase();
        ...
        support_material: { key: 'support_enable', fn: function fn(v) {
                return !!parseInt(v);
            } },
        support_material_spacing: { key: 'support_line_distance' },
        support_material_threshold: { key: 'support_angle', fn: function fn(v) {
                return 90.0 - parseFloat(v);
            } },
        support_material_pattern: { key: 'support_pattern', fn: function fn(v) {
                v = v.toLowerCase();
        ...
        support_material_speed: { key: ['speed_support_infill', 'speed_support_interface'], fn: function fn(v) {
        ...
        temperature: { key: ['temperature', 'material_print_temperature'], fn: function fn(v) {
                return [parseFloat(v), parseFloat(v)];
            } },
        layer_height: { key: ['layer_height', 'infill_sparse_thickness'], fn: function fn(v) {
                return [parseFloat(v), parseFloat(v)];
            } },
    ...
    var logError = function logError() {
        console.error('Cura Rev Function not implemented', i);
    };
    for (var _i in CONFIG_FLUX_TO_CURA) {
        var item = CONFIG_FLUX_TO_CURA[_i];
        CONFIG_CURA_TO_FLUX[item.key] = { key: _i };
        if (item.fn) {
            CONFIG_CURA_TO_FLUX[item.key].fn = logError;
        }
    }

    CONFIG_CURA_TO_FLUX.support_angle.fn = function (v) {
        return 90 - v;
    };
    CONFIG_CURA_TO_FLUX.support_enable.fn = function (v) {
        return v === true || v === 1 || v === 'true' ? 1 : 0;
    };
    CONFIG_CURA_TO_FLUX.speed_support_infill = { key: 'support_material_speed' };
    CONFIG_CURA_TO_FLUX.infill_line_distance = { key: 'fill_density', fn: function fn(v, settings) {
            return Math.min(100, 40 / parseFloat(v));
        } };

    CONFIG_CURA_TO_FLUX.support_pattern.fn = function (v) {
        return v.toUpperCase();
    };
    CONFIG_CURA_TO_FLUX.infill_pattern.fn = function (v) {
        return v.toUpperCase();
    };
    ...
    function SlicerConfig(id, opts) {
        ...
        this.id = id || s4() + s4() + '-' + s4() + '-' + s4() + '-' + s4() + '-' + s4() + s4() + s4();
        // Default Config
        this.config = {
            engine: 'cura2',
            temperature: 200,
    ...
    SlicerConfig.prototype.getConfigStr = function () {
        var customString = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
        ...
        Object.keys(self.config).map(function (key) {
            ...
            if (CONFIG_FLUX_TO_CURA[key] && CONFIG_FLUX_TO_CURA[key].key) {
                var _item = CONFIG_FLUX_TO_CURA[key];

                if (_item.key instanceof Array) {
                    _item.key.map(function (v, i) {
                        value = _item.fn ? _item.fn(value, self)[i] : value;
                        insertConfig(newConfigsStr, v, value);
                    });
                    return;
                } else {
                    key = _item.key;
                    value = _item.fn ? _item.fn(value, self) : value;
                }
            }

            // support_material is for slic3r and cura, we manually reject
            if (CURA2_CONFIG_KEYS.indexOf(key) >= 0) {
    ...
    SlicerConfig.prototype.load = function (settings) {
        var withCustom = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        ...
        if ((typeof settings === 'undefined' ? 'undefined' : _typeof(settings)) === 'object') {
            if (settings.defaultSetting) {
                ...
                if (this.configStr === null) {
                    throw new Error('null configStr error');
                }
            } else {
                var holdAttrs = { id: settings.id };
                if (!withCustom) {
                    holdAttrs = {
                        id: settings.id,
            ...
            settings.forEach(function (line) {
                ...
                if (item.length === 2) {
                    var _key = item[0].replace(/ /g, ''),
                        _value = item[1].trim();

                    var configValueTranslator = CONFIG_CURA_TO_FLUX[_key];
    ...
    SlicerConfig.prototype.filter = function (p0) {
        ...
        if (CONFIG_FLUX_TO_CURA[param.key] && CONFIG_FLUX_TO_CURA[param.key].key) {
            var _item2 = CONFIG_FLUX_TO_CURA[param.key];
            if (_item2.key instanceof Array) {
                param.key = _item2.key;
                param.value = [];
                _item2.key.map(function (v, i) {
                    param.value[i] = _item2.fn ? _item2.fn(p0.value, self)[i] : p0.value;
                });
            } else {
                param.key = _item2.key;
                param.value = _item2.fn ? _item2.fn(param.value, self) : param.value;
            }
        }
        return param;
    };
    ...
    SlicerConfig.prototype.deepClone = function () {
        var clone = new SlicerConfig(this.id + '-clone');
