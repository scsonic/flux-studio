 ...
 * setting up shortcut
 */
define(['jquery', 'app/constants/keycode-constants', 'helpers/array-findindex'], function ($, KeycodeConstants) {
    'use strict';

    var root = window,
        isMetaKey = function isMetaKey(keyCode) {
        ...
        keyCodeStatus = [],
        has_bind = false,
        keyup_event = function keyup_event(e) {
        keyCodeStatus = [];
    },
        keydown_event = function keydown_event(e) {
        var matches = [];
        ...
        convertToKeyCode = function convertToKeyCode(keys) {
        ...
        generateKey = function generateKey(keyCodes) {
        return keyCodes.sort().join('+');
    },
        matchedEvents = function matchedEvents(keyCodes) {
        ...
        _unique = function _unique(arr) {
        //this is array-unique.js
        var unique_array = [],
            some = function some(compare) {
            return function (el) {
                return compare === el;
            };
        };
    ...
    return {
        on: function on(keys, callback) {
            var keyCodes = convertToKeyCode(keys);
            var e = { key: keys, keyCode: generateKey(keyCodes), callback: callback };
            events.push(e);

            initialize();
            return unsubscribe.bind(e);
        },
        off: function off(keys) {
            var keyCodes = convertToKeyCode(keys),
